...
<h2 className="font-bold font-mono text-pink-400 text-center">Select Quantity</h2>
      ...
      <div className="box-flex product-tile__right-side-wrapper no-shrink ml-st pt-8">
        ...
        <button
          ...
          <div className='font-thin text-lg px-4 mt-1'>{quantity}</div>
          <button className='font-bold	 text-xl color-pink pr-12 text-pink-500'
            onClick={() => setQuantity(quantity + 1)}
          >
            +
          </button>
      ...
      <Link className="btn btn-secondary hover:scale-105 transition-all font-sans md:font-serif absolute bottom-0 right-0 p-2" href="./cart" >Add to cart</Link>
      </div>
      </div>
     ...
     <div className="modal-action">
      <form method="dialog">
        <button className='absolute bottom-40 right-2 rounded-full'>
